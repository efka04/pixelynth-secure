...
// Si ce n'est pas déjà fait dans index.js, décommentez la ligne suivante
// admin.initializeApp();

/**
 * Fonction Firebase qui se déclenche lorsqu'une image est téléchargée dans le bucket Storage
 * Cette fonction compresse l'image en format WebP avec les paramètres spécifiés
 */
exports.compressImage = functions.storage.object().onFinalize(async (object) => {
  ...
  try {
    ...
    // Vérifier si l'image est associée à un document Firestore
    // Cette partie est optionnelle et dépend de votre structure de données
    // Vous devrez peut-être l'adapter à votre cas d'utilisation spécifique
